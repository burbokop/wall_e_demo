    ...
    if(args.size() > 0) {
        if(args[0].contains_type<wall_e::lex::token>()) {
            ...
            if(token.name == "INT_LITERAL") {
                ...
                return new arg_node(StringLiteral, wall_e::lex::parse_string_literal(token.text, true));
            } else if(token.name == "TOK_ID") {
                return new arg_node(Id, token.text);
            }
        } else {
            const auto node = args[0].option_cast<km2::abstract_value_node*>();
            if (node.has_value()) {
                return new arg_node(ValueNode, {}, node.value());
            }
        }
    }
