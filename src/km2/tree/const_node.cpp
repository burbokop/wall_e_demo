...
std::list<km2::error> km2::const_node::errors() {
    return {};
}

wall_e::either<km2::error, llvm::Value *> km2::const_node::generate_llvm(const std::shared_ptr<module> &module) {
    if(const auto value = m_value->generate_llvm(module)) {
        ...
        if(status == module::ArgSettingSuccess) {
            ...
            return wall_e::left(error("ArgSettingDublicates"));
        } else if(status == module::ArgSettingEmptyStack) {
            return wall_e::left(error("ArgSettingEmptyStack"));
        } else {
            return wall_e::left(error("ArgSettingUnknownStatus"));
        }
    } else {
        return value.left();
    }
